...
// for (let index = 0; index < amountOfCourses; index++) {
...
// let coursesFound = []
// coursesFound = courses.filter()
let specificCode= courses.filter(p => p.CourseCode===("AK-47"));
   if (specificCode.length > 0) {
  for (let i = 0; i < specificCode.length; i++) {
 console.log(specificCode[i].Price);
 }}




const affordableCourses1 = courses.filter(course => parseFloat(course.Price) < 50.00);
console.log(affordableCourses1);